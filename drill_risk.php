...
$drill_risk_status = $_GET['occur_status'] ?? '';
   ...
   // Initialize filter variables with defaults
    ...
    //echo "<pre>";
    //echo "Debug: Session Filters\n";
    //print_r($filters);
    //echo "Drill Risk Status: " . htmlspecialchars($drill_risk_status) . "\n";
    //echo "</pre>";
...
<?php
if (isset($_SESSION['filters'])) {
    // Modified query to match the first query's logic while maintaining drill-down functionality
    $sql = "SELECT *  
            FROM occur
            WHERE 1=1"; // Start with always true condition to make adding conditions easier
    
    // Only add the specific status filter if it's not "No QA/Risk Mgmt Status"
    if ($drill_risk_status === "No QA/Risk Mgmt Status") {
        $sql .= " AND (occur_status IS NULL OR TRIM(occur_status) = '')";
    } else {
        $sql .= " AND occur_status = '" . mysqli_real_escape_string($conn, $drill_risk_status) . "'";
    }
    
    // Add the closed status exclusion to match first query
    $sql .= " AND (occur_status != 'Closed' OR occur_status IS NULL)";
    ...
    // Handle "No QA/Risk Mgmt Status" case
    if ($drill_risk_status === "No QA/Risk Mgmt Status") {
        $sql .= " AND (occur_status IS NULL OR TRIM(occur_status) = '')";
    } else {
        $sql .= " AND occur_status = '" . mysqli_real_escape_string($conn, $drill_risk_status) . "'";
...
if (isset($_SESSION['filters'])) {
    $datatable_name = "Pending QA / Risk Management Review " . htmlspecialchars($drill_risk_status);
} else {
    $datatable_name = "Manager Detail: " . htmlspecialchars($drill_risk_status);
}
?>
