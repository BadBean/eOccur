...
// Query to get unique categories from the occur_location field ----------------------------------
...
if (isset($_GET['month']) || isset($_GET['location']) || isset($_GET['patient_loc']) || 
    ...
    isset($_GET['occur_area']) || isset($_GET['manager_followup_name'])) {    // Added this condition
    ...
    $_SESSION['filters'] = [
        ...
        'md' => $selected_md,
        'area' => $selected_area, 
        'mgr' => $selected_mgr // Added manager criteria here
    ];
}
                            ...
                            <div class="col-md-12 col-xl-12">
                                <div class="card" style="background-color: #DCDCDC;">
                                    <div class="card-body text-center text-black">
                                        ...
                                        <h4>FOLLOW UP / ACTION PLAN REVIEW</h4>
                                    </div>
                                </div>
                            </div>
                        </div> <!-- end row -->
...
<div class="row mt-4">
    ...
    <!-- PHP / SQL QUERY AND HTML FOR "MANAGER FOLLOW-UP DETAIL" - COLUMN 3 ============================ -->

    <?php
    // Query to count the number of reports by manager follow-up within the desired date range
    $sql_mgr_table = "SELECT manager_followup_name, COUNT(*) as mgr_table_count 
                      FROM occur 
    ...
    $sql_mgr_table .= " GROUP BY manager_followup_name
                        ORDER BY mgr_table_count DESC";
    ...
    while ($row = mysqli_fetch_assoc($result_mgr_table)) {
        ...
        if (!isset($mgr_table_styles[$manager_name])) {
            $mgr_table_styles[$manager_name] = [
    ...
    mysqli_data_seek($result_mgr_table, 0);
    ?>

    <div class="col-md-3">
        <?php if (mysqli_num_rows($result_mgr_table) > 0) : ?>
            <div class="card h-100">
                <div class="card-body pb-2">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <h5 class="card-title me-2">MANAGER FOLLOW-UP REQUIRED</h5>
                    </div>

                    <div class="mt-4 text-center">
                        <div>
                            <?php while ($row_mgr_table = mysqli_fetch_assoc($result_mgr_table)) :
                                $manager_followup_name = htmlspecialchars($row_mgr_table['manager_followup_name']);
                                $count_mgr_table = htmlspecialchars($row_mgr_table['mgr_table_count']);
                                $total_mgr_table_count += $row_mgr_table['mgr_table_count'];
                                $color_mgr_table = $mgr_table_styles[$manager_followup_name]['color'];
                            ?>
                                <div class="d-flex justify-content-between align-items-center border-bottom pb-2">
                                    <div class="d-flex">
                                        <i class="mdi mdi-circle font-size-12 mt-1 <?= $color_mgr_table ?>"></i>
                                        <div class="flex-grow-1 ms-2">
                                            <p class="mb-0"><?= $manager_followup_name ?></p>
                                        </div>
                                    </div>
                                    <div>
                                        <h5 class="mb-0 font-size-14"><a href="drill_manager.php?manager_followup_name=<?= urlencode($manager_followup_name) ?>"><?= $count_mgr_table ?></a></h5>
                                    </div>
                                </div>
                            <?php endwhile; ?>
                            ...
                            <div class="d-flex justify-content-between align-items-center pt-2">
                                ...
                                <div>
                                    <h5 class="mb-0 font-size-14"><strong><?= htmlspecialchars($total_mgr_table_count) ?></strong></h5>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <?php else : ?>
            <div class="card h-100">
                <div class="card-body pb-2">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <h5 class="card-title me-2">MANAGER FOLLOW-UP DETAIL</h5>
...
<div class="row mt-4">
    <div class="col-md-12 col-xl-12">
        <div class="card" style="">
            <div class="card-body text-center text-black <div class= alert alert-info">
                <h5 class="mb-sm-0 fw-bold"></h5>
