...
<!-- SET VARIABLES / MONTH  ================ -->
			...
			<?php
				// Check if the 'selected_month' parameter is set
                if (isset($_GET['selected_month'])) {
                    // Get the value of 'selected_month'
                    ...
                    // Format current month to display as full word in datatable title only if $selected_month is set
                    $format_selected_month_name = date('F', strtotime($selected_month . '-01'));
                } else {
...
// QUERY FOR DATATABLE  ================ -->
        ...
        $sql =  "SELECT *
                 FROM occur
                 WHERE occur_date LIKE '$selected_month%'
...
// TITLE AND SUBTITLES FOR DATATABLE  ================ -->
        ...
        $datatable_name = "Area Detail:&nbsp&nbsp " . htmlspecialchars($occur_area);
