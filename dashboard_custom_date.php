...
<div class="row">
   ...
   $type_styles = [
    ...
    if (mysqli_num_rows($result_type_table) > 0) {
        ...
        echo '            </div>';
        echo '            <h5 class="card-title mb-3 me-2">Total by Type:&nbsp&nbsp' . date('F Y', strtotime($selected_month)) . '</h5>';
...
echo '                                <a href="drill_loc.php?patient_loc=' . urlencode($patient_loc_table) . '&month=' . urlencode($selected_month) . '">';
...
echo '                    </div>';
    ...
    if (mysqli_num_rows($result_location_table) > 0) {
        ...
        while ($row_location_table = mysqli_fetch_assoc($result_location_table)) {
            ...
            echo '                        <div>';
            echo '                            <h5 class="mb-0 font-size-14">
                                                <a href="drill_location.php?occur_location=' . urlencode($occur_location_table) . '&month=' . urlencode($selected_month) . '">' . $count_location_table . '</a></h5>';
            echo '                        </div>';
            echo '                    </div>';
        }
    ...
    if (mysqli_num_rows($result_program_table) > 0) {
        ...
        while ($row_program_table = mysqli_fetch_assoc($result_program_table)) {
            ...
            echo '                        </div>';
            echo '                        <div>';
            echo '                            <h5 class="mb-0 font-size-14"><a href="drill_program.php?patient_program=' . urlencode($patient_program_table) . '&month=' .      
        ...
        chart: {
            ...
            events: {
                dataPointSelection: function(event, chartContext, config) {
                    ...
                    window.location.href = 'dashboard_category.php?reporter_category=' + encodeURIComponent(selectedCategory) + '&month=' + encodeURIComponent(selectedMonth);
                }
            }
        },
        plotOptions: {
